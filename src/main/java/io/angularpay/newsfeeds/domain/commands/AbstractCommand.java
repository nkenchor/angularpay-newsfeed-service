...
import io.angularpay.newsfeeds.exceptions.ValidationException;
...
import static io.angularpay.newsfeeds.exceptions.ErrorCode.*;
...
public abstract class AbstractCommand<T extends AccessControl, R> {
    ...
    public R execute(T request) {
        try {
            ...
            if (StringUtils.hasText(resourceOwner) && StringUtils.hasText(request.getAuthenticatedUser().getUserReference())) {
